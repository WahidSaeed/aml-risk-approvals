...
This single-file CloudFormation template (`AML-Streaming-Core-Stack-portable.yaml`) defines your serverless resource footprint. Hardcoded properties have been stripped in favor of dynamic Pseudo Parameters (`${AWS::AccountId}`, `${AWS::Region}`) to support automated, cross-account deployment pipelines.
...
1. **Deploy Backend Infrastructure:** Upload the `AML-Streaming-Core-Stack-portable.yaml` to AWS CloudFormation in your designated region, ensuring your Lambda code packages are present in your target deployment S3 bucket.
